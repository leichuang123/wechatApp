...
Page({
    ...
    getInfo(type) {
        ...
        const param = {
            ...
            start: prevPage.data.start,
            end: prevPage.data.end,
            travel_id: prevPage.data.travel_id,
        ...
        api.get('weapp-obd-user-car/single-travel', param).then(res => {
            ...
            if (self.data.type == 2) {
                self.setData({
                    all: '总油耗' + res.data.total.total_oil,
            ...
            if (self.selectComponent('#pieSelects')) {
                ...
                self.chartComponent.init((canvas, width, height) => {
                    var data = [
                        ...
                        { name: '快速变道', sales: self.data.describe.RapidLaneChange },
                        { name: '急转弯', sales: self.data.describe.SharpTurn },
                        { name: '急刹车', sales: self.data.describe.SlamTheBrakesOn },
